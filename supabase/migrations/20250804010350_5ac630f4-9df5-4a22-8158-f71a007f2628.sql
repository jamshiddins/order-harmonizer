...
-- Исправляем функции, добавляя search_path
...
-- Пересоздаем представления без SECURITY DEFINER (по умолчанию SECURITY INVOKER)
DROP VIEW IF EXISTS v_complete_orders;
DROP VIEW IF EXISTS v_problematic_orders;
DROP VIEW IF EXISTS v_file_processing_stats;

-- Создаем обычные представления (будут использовать права вызывающего)
